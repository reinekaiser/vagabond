...
const updateCity = async (req, res) => {
    try {
        const { cityId } = req.params;
        // Nếu là multipart/form-data (có file)
        if (req.files && req.files.length > 0) {
            // Lấy các trường text từ req.body
            const { name, description, bestTimeToVisit } = req.body;
            // Xử lý ảnh city
            let cityImages = [];
            const cityImageFiles = req.files.filter(file => file.fieldname === 'images');
            for (const file of cityImageFiles) {
                const imageUrl = await uploadToCloudinary(file, 'cities');
                cityImages.push(imageUrl);
            }
            ...
            if (req.body.oldImages) {
                if (Array.isArray(req.body.oldImages)) cityImages = [...cityImages, ...req.body.oldImages];
                else cityImages.push(req.body.oldImages);
            }
            // Xử lý popularPlace
            const popularPlaces = [];
            const popularPlaceData = {};
            ...
            const popularPlaceFiles = req.files.filter(file => file.fieldname.startsWith('popularPlaces') && file.fieldname.includes('.img'));
            for (const file of popularPlaceFiles) {
                const match = file.fieldname.match(/popularPlaces\[(\d+)\]\.img/);
                if (match) {
                    const index = match[1];
